...
# Form Assistant Tutorial

**AI Form Toolkit** has tools to enhance your application's forms with an AI assistant that autofills form fields.
  ...
  return (
    <>
      <textarea
        ...
        onChange={e => setFormValues({...formValues, recipe: e.target.value})}
      />
      <button type="button" onClick={() => fillSingleField('recipe')}>Autofill with AI</button>
    </>
  );
}
```
...
function FormWithAssistant() {
  ...
  return (
    <>
      ...
      <button type="button" onClick={() => fillFields()}>Autofill with AI</button>
    </>
  );
}
```
...
Check [the examples](/examples/#form-assistant-examples) for some ideas.
...
<Grid>
  ...
  <Grid.Col span={6}><Button fullWidth variant="outline" component={Link} href="/examples/">All Examples »</Button></Grid.Col>
</Grid>

...
# Form Generation Tutorial

**AI Form Toolkit** integrates with [OpenAI's GPT models](https://platform.openai.com/docs/guides/text-generation)
...
**AI Form Toolkit** can generate forms from any kind of text content: an article, a task description, a legal contract, a list of topics, etc.
Even if the text is code, like [Mermaid diagrams](https://mermaid.js.org/).
As long as it's text, and the AI can understand it, **AI Form Toolkit** can generate a form for it.
...
import {
  useGeneratedFormSchema
} from 'ai-form/hooks/form-gen';
...
**AI Form Toolkit** provides a React hook, `useGeneratedFormSchema`, which takes no arguments.
...
- You can pre-fill the content and prompt and enable the user to edit only part of it.
- Perhaps fill the content based on the application context, and let the user only set the prompt.
- Or even set both depending on what your user is doing.

Check [the examples](/examples/#form-generation-examples) for some ideas.
...
<Grid>
  <Grid.Col span={6}><Button fullWidth variant="outline" component={Link} href="/tutorial/getting-started/">« Getting Started</Button></Grid.Col>
  <Grid.Col span={6}><Button fullWidth variant="outline" component={Link} href="/tutorial/form-assistant/">Form Assistant »</Button></Grid.Col>
</Grid>
